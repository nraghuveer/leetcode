# given list of non-negative integers, form a largest number from given integers
from typing import List
from functools import cmp_to_key


class Solution:
    def largestNumber(self, nums: List[int]) -> str:
        def cmp(x, y):
            u = x + y
            v = y + x
            if u == v:
                return 0
            elif u < v:
                return 1
            else:
                return -1

        nums = sorted(map(str, nums), key=cmp_to_key(cmp))
        result = "".join(nums)
        result = result.lstrip("0")
        return "0" if not result else result


if __name__ == "__main__":
    s = Solution()
    # assert s.largestNumber([0, 0]) == "0"
    # assert s.largestNumber([1]) == "1"
    # assert s.largestNumber([121, 12]) == "12121"
    assert s.largestNumber([9, 5, 34, 3, 30]) == "9534330"
    assert s.largestNumber([3, 30, 34, 5, 9]) == "9534330"
    assert s.largestNumber([10, 2]) == "210"
    assert s.largestNumber([1, 1, 1, 1, 1]) == "11111"
    assert s.largestNumber([3, 3, 31, 30, 35]) == "35333130"
    assert s.largestNumber([100, 101, 102, 110, 120]) == "120110102101100"
    assert (
        s.largestNumber([824, 938, 1399, 5607, 6973, 5703, 9609, 4398, 8247])
        == "9609938824824769735703560743981399"
    )
    assert (
        s.largestNumber(
            [
                4704,
                6306,
                9385,
                7536,
                3462,
                4798,
                5422,
                5529,
                8070,
                6241,
                9094,
                7846,
                663,
                6221,
                216,
                6758,
                8353,
                3650,
                3836,
                8183,
                3516,
                5909,
                6744,
                1548,
                5712,
                2281,
                3664,
                7100,
                6698,
                7321,
                4980,
                8937,
                3163,
                5784,
                3298,
                9890,
                1090,
                7605,
                1380,
                1147,
                1495,
                3699,
                9448,
                5208,
                9456,
                3846,
                3567,
                6856,
                2000,
                3575,
                7205,
                2697,
                5972,
                7471,
                1763,
                1143,
                1417,
                6038,
                2313,
                6554,
                9026,
                8107,
                9827,
                7982,
                9685,
                3905,
                8939,
                1048,
                282,
                7423,
                6327,
                2970,
                4453,
                5460,
                3399,
                9533,
                914,
                3932,
                192,
                3084,
                6806,
                273,
                4283,
                2060,
                5682,
                2,
                2362,
                4812,
                7032,
                810,
                2465,
                6511,
                213,
                2362,
                3021,
                2745,
                3636,
                6265,
                1518,
                8398,
            ]
        )
        == ""
    )
